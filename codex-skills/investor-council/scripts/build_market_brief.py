...
from session_memory import load_memory, memory_file_for_mentor
...
def build_brief(mentor_id: str, query: str, market_notes: str, product_home: str = '') -> dict[str, Any]:
    client = MarketDataClient()
    memory = load_memory(memory_file_for_mentor(mentor_id, product_home))
...
def _to_markdown(brief: dict[str, Any]) -> str:
    lines = ['# Market Brief']
    lines.append(f"- Mentor: {brief['mentor_id']}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Build a live market brief for one mentor in investor council')
    ...
    brief = build_brief(args.mentor_id, args.query, args.market_notes, args.product_home)
