...
QUOTE_HIGH_COL = '最高'
QUOTE_LOW_COL = '最低'
QUOTE_PREV_CLOSE_COL = '昨收'
QUOTE_TURNOVER_COL = '换手'
QUOTE_AMOUNT_COL = '金额'
QUOTE_VOLUME_COL = '总手'
INFO_INDUSTRY_COL = '行业'
...
class MarketDataClient:
    ...
    def available(self) -> bool:
        return ak is not None or requests is not None
    ...
    def get_market_overview(self) -> dict[str, Any]:
        ...
        if ak is not None:
            try:
                ...
                for index_name in CN_INDEX_NAMES:
                    matched = df[df[INDEX_NAME_COL].astype(str) == index_name]
                    if matched.empty:
                        continue
                    row = matched.iloc[0]
                    rows.append(
                        {
                            'name': str(row[INDEX_NAME_COL]),
                            'price': self._to_float(row[INDEX_PRICE_COL]),
                            'pct': self._to_float(row[INDEX_PCT_COL]),
                            'change': self._to_float(row[INDEX_CHANGE_COL]),
                        }
                    )
                if rows:
        ...
        message = '实时行情暂时不可用。' if not errors else f"实时行情暂时不可用：{'; '.join(errors)}"
    ...
    def get_stock_snapshot(self, text: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            'message': '; '.join(errors),
            'name': str(info_map.get(INFO_NAME_COL) or fallback_map.get('name') or resolved['name']),
            'code': code,
            'latest': self._to_float(quote_map.get(QUOTE_LATEST_COL) or info_map.get(INFO_LATEST_COL) or fallback_map.get('latest')),
            'pct': self._to_float(quote_map.get(QUOTE_PCT_COL) or fallback_map.get('pct')),
            'change': self._to_float(quote_map.get(QUOTE_CHANGE_COL) or fallback_map.get('change')),
            'open': self._to_float(quote_map.get(QUOTE_OPEN_COL) or fallback_map.get('open')),
            'high': self._to_float(quote_map.get(QUOTE_HIGH_COL) or fallback_map.get('high')),
            'low': self._to_float(quote_map.get(QUOTE_LOW_COL) or fallback_map.get('low')),
            'prev_close': self._to_float(quote_map.get(QUOTE_PREV_CLOSE_COL) or fallback_map.get('prev_close')),
            'turnover': self._to_float(quote_map.get(QUOTE_TURNOVER_COL) or fallback_map.get('turnover')),
            'amount': self._to_float(quote_map.get(QUOTE_AMOUNT_COL) or fallback_map.get('amount')),
            'volume': self._to_float(quote_map.get(QUOTE_VOLUME_COL) or fallback_map.get('volume')),
            'industry': str(info_map.get(INFO_INDUSTRY_COL) or fallback_map.get('industry') or ''),
        }
